now we are going to create the mechanism for the Rental Truck Negotiation having the same mechanism as trucks negotiation which has been implemented.
# Rental Truck Negotiation
## Backend
...
3. In the select delivery type if the user selects Seller delivery then below the buttons, 1 city dropdown, 1 text input, 1 number input for Buyer Delivery City and Buyer Delivery Address and delivery cost (as per the Rental Truck Negotiation negotiation schema) should become visible.
...
5. when clicking on submit offer button then it should open a bootstrap confirmation modal saying that are you sure you want to submit an offer etc. 
   1. When the user clicks on submit then it should create a Rental Truck Negotiation negotiation with the provided information.
   2. make sure the UI of the confirmation modal matches the UI and theme of the website.
   3. after clicking the submit button the Rental Truck Negotiation negotiation document should be created as follow:
      1. the create Rental Truck Negotiation negotiation endpoint uses the authtoken middleware by which we can get the user id and use it in the buyer field.
      2. the seller id comes from the respective Rental Truck Negotiation listing document.
         1. from the frontend the Rental Truck Negotiationid is sent in the req.body, so inside the endpoint using the Rental Truck Negotiationid you retrieve the Rental Truck Negotiation document from whcih you can retrieve the sellerid.
      3. the Rental Truck Negotiationid is sent in the req.body from the frontend
      ...
      6. at the time of Rental Truck Negotiation negotiation creation, in the negotiation array, 1 object will be added where
         1. negotiator will be 'buyer'
         2. perDayRentalCharges will be the amount that is taken from the Offer Rental Truck Per Day Amount from the offer form.
      ...
      7. the rest of the fields of the Rental Truck Negotiation negotiation document should be null/default
   4. when a negotiation is submitted then it should navigate the user to the Rental Truck Negotiation-negotiation/:Rental Truck NegotiationnegotiationID dynamic route of the user dashboard.
   ...
   Here it will render all the negotiations of the negotiation array of the Rental Truck Negotiation negotiation object.
   ...
   7. Clicking the counter offer button will open a bootstrap model for Submit Cuounter offer, which consists of the following inputs:
      1. Rental Truck Per Day Negotiation Cost Number input
   ...
   11. In the Pay Advance Fee Section:
       ...
       2. Show the Rental Truck Negotiation Cost and Delivery Cost.
       ...
       9. In this final payment section it will show the following amounts:
          1. Agreed Rental Truck Negotiation Security Deposit Cost=securityDepositAmount
          ...
          3. Agreed Total Cost = Rental Truck Security Deposit Negotiation Amount+Delivery
       ...
       10. once the finalPaymentStatus is approved then it should update the purchaseOrderDate to the current date and it will show negotiation successful and a Purchase Order will be visible which consist of the following informations:
          1. The Strucutre of the Rental Truck Negotiation Purchase Order should be similar to the reference purchase order image ![reference purchase order](image.png).
         ...
         7. The item table should consist of the following columns:
            1. Title (render the Rental Truck Negotiation title)
            2. Category (render the category name of the Rental Truck Negotiation)
            3. Rental Truck Brand
            4. Rental Truck Security Deposit
...
2. The UI of the Negotiations page should be similar to the Ui of View listings, when the Admin clicks on the Rental Truck Negotiations Negotiation then it should navigate the admin to the Rental Truck Negotiation-neogtiations page.
3. The UI of Rental Truck Negotiation Negotiations page is also similar to Rental Truck Negotiations Lisitng page whcih will render all the negotiations of all the Users.
4. Clicking on the view button of a particular Rental Truck Negotiation negotiation will take the admin to the negotiation-detail/:Rental Truck NegotiationNegotiationID.
5. The UI of the Rental Truck Negotiation negotiation detail page will follow the UI of the admin dashboard where it will render all the information of the respective Rental Truck Negotiation negotiation.
