# Machinery Inspection Service Negotiation Report
## Backend
1. 1. in the models folder create a new schema for Machinery Inspection Service Negotiation Report negotiation which consist of the following fields:
   1. inspectionRequester: id ref(user)
   2. inspector: id ref(user)
   3. machineryInspectionNegotiation: id ref (machineryInspectionNegotiation)
...
2. in the routes folder, create the respective route file for machineryInspectionReport to manage its crud endpoints.
...
2. Clicking on the Machinery Inspection Service will take the user to the Machinery Inspection Service page, whose ui and other things will be similar to the truck listings page where it will show all the Machinery Inspection Service Negotiations whose (inspectionRequester id matches the logged in user and the machineryInspectionReport.machineryInspectionNegotiation.finalPaymentStatus is paid) or inspector id matches the id of the logged in user.
   1. At the top right of the Machinery Inspection Service Negotiation page, there will be a create report button which takes the user to the /create-truck-inspection-report page.
   The Create Machinery Inspection Service Negotiation Report page ui and mechanism should be as follows:
   1. It shows all the sections and inputs as per the machineryInspectionReport schema,
      1. at the start of the inspection form, there will be a search and select for select Machinery Inspection Service Negotiation negotiation the same way as it is used for product type in [transaction.jsx](/Users/mac/gitRepos/sbc-marketplaced/Custom-ReactJS-Workflow/referenceRepo/referenceComponent/transaction.jsx).
         1. it fetches and shows those Machinery Inspection Service Negotiation negotiations whose seller id matches the id of the logged in user and the advance status is paid.
         and the machineryInspectionReport id is null
         2. it shows the results whose buyer.fullname or buyer.username matches the search query. (it should be broad search)
      2. selecting a negotiation in the select Machinery Inspection Service Negotiation negotiation will use the buyer._id for the inspectionRequester field and use the selected negotiation id for the machineryInspectionNegotiation field from the selected negotiation.
   ...
   4. Clicking on submit button will submit the Machinery Inspection Service Negotiation report and also adds & update the id of the newly created Machinery Inspection Service Negotiation report to the selected Machinery Inspection Service Negotiation negotiation document.
3. In the Machinery Inspection Service Negotiation page, if a user selects a Machinery Inspection Service Negotiation then if the logged in user is a inspectionRequester then only the report is visible to him.
...
1. In the admin dashboard add another tab of Inspection same as Listings, where it shows all the types of inspection cards which include:
   1. Machinery Inspection Service Negotiation
   2. Machinery Inspection
   3. Spare Part Inspection
   4. Material Inspection
2. Clicking on the Machinery Inspection Service Negotiation will take the user to the Machinery Inspection Service Negotiation page, whose ui and other things will be similar to the truck listings page where it will show all the Machinery Inspection Service Negotiations.
...
now we are going to create the mechanism for the Machinery Inspection Service Negotiation Service having the same mechanism as trucks negotiation which has been implemented.
# Machinery Inspection Service Negotiation Service Negotiation
## Backend
1. in the models folder create a new schema for Machinery Inspection Service Negotiation Service negotiation which consist of the following fields:
   1. buyer: id ref(user)
   ...
   14. machineryInspectionReport: id ref(machineryInspectionReport)
   15. purhcaseOrderDate: Date
2. in the routes folder, create the respective route file for machineryInspectionNegotiation to manage its crud endpoints.
## Frontend
## User Dashboard
...
4. Clicking on the Machinery Inspection Service Negotiation negotiation will navigate the user to the Machinery Inspection Service Negotiation negotiation page.
5. The UI of Machinery Inspection Service Negotiation Service Negotiations page is also similar to Machinery Inspection Service Negotiation Services Lisitng page.
In the Machinery Inspection Service Negotiation Services negotiation page it will fetch and show all the negotiations whose buyer or seller field matches the logged in user id.
4. Clicking on the view button will take the user to the /inspection-service-negotiation/:machineryInspectionNegotiationID dynamic route which is described below in next point.

## Inspection Service Details Page
1. [ Inspection ServiceDetailsSidebarCards.jsx](/Users/mac/gitRepos/sbc-marketplaced/Custom-ReactJS-Workflow/src/components/marketplace/Machinery Inspection Service Negotiation ServiceDetailsSidebarCards.jsx) write Offer Machinery Inspection Service Negotiation Service Amoount instead of Offer Amount.
...
If the selected product category.categoryType is machinery then negotiation for the machineryInspectionNegotiation will be created when clicking on submit request.
...
4. In the select delivery type if the user selects On-Site then below the buttons, 1 city dropdown, 1 text input for Buyer Delivery City and Buyer Delivery Address (as per the Machinery Inspection Service Negotiation Service negotiation schema) should become visible.
...
6. when clicking on submit offer button then it should open a bootstrap confirmation modal saying that are you sure you want to submit an offer etc. 
   1. When the user clicks on submit then it should create a Machinery Inspection Service Negotiation Service negotiation with the provided information.
   2. make sure the UI of the confirmation modal matches the UI and theme of the website.
   3. after clicking the submit button the Machinery Inspection Service Negotiation Service negotiation document should be created as follow:
      1. the create Machinery Inspection Service Negotiation Service negotiation endpoint uses the authtoken middleware by which we can get the user id and use it in the buyer field.
      2. the seller id comes from the respective Machinery Inspection Service Negotiation Service listing document.
         1. from the frontend the Machinery Inspection Service Negotiation Serviceid is sent in the req.body, so inside the endpoint using the Machinery Inspection Service Negotiation Serviceid you retrieve the Machinery Inspection Service Negotiation Service document from whcih you can retrieve the sellerid.
      3. the Machinery Inspection Service Negotiation Serviceid is sent in the req.body from the frontend
      ...
      6. at the time of Machinery Inspection Service Negotiation Service negotiation creation, in the negotiation array, 1 object will be added where
         1. negotiator will be 'buyer'
         2. labourCharges will be 0
         
      7. the rest of the fields of the Machinery Inspection Service Negotiation Service negotiation document should be null/default
   4. when a negotiation is submitted then it should navigate the user to the Machinery Inspection Service Negotiation Service-negotiation/:Machinery Inspection Service Negotiation ServicenegotiationID dynamic route of the user dashboard.
   ...
   Here it will render all the negotiations of the negotiation array of the Machinery Inspection Service Negotiation Service negotiation object.
   ...
   11. In the Pay Advance Fee Section:
       ...
       2. Show the Machinery Inspection Service Negotiation Service Cost and Delivery Cost.
       ...
       8. once the status has been changed to paid by the admin then this section will now become invisible just like negotiations and now the Machinery Inspection Service Negotiation report status section will be visible which shows the status pending if there is no id assigned to the machineryInspectionReport field of the machineryInspectionNegotiation.

9. when the id to the machineryInspectionReport is assigned then the status will be changed to pending approval following the status of the machineryInspectionNegotiation.machineryInspectionReport.status
       
10. when the machineryInspectionReport.status is changed to approved then the Final Payment section will be visible following the UI of the figma design: [SBC Marketplace UI 1.0 Copy](https://www.figma.com/design/uJ82aUAo9qF9dUIBSw4lNP/SBC-Marketplace-UI-1.0--Copy-?node-id=253-3171&t=Q68W0nWhC32fdKID-4).
...
9. In this final payment section it will show the following amounts:
   1. Agreed Machinery Inspection Service Negotiation Service Cost=labourCharges
   2. Agreed Total Cost = Machinery Inspection Service Negotiation Service+Delivery
...
10. once the finalPaymentStatus is approved then it should update the purchaseOrderDate to the current date and it will show negotiation successful and a Purchase Order will be visible which consist of the following informations:
    1. The Strucutre of the Machinery Inspection Service Negotiation Service Purchase Order should be similar to the reference purchase order image ![reference purchase order](image.png).
    ...
    7. The item table should consist of the following columns:
       1. Title (render the Machinery Inspection Service Negotiation Service title)
       2. Category (render the category name of the Machinery Inspection Service Negotiation Service)
       3. Machinery Inspection Service Negotiation Service cost
...
11. before the purchase order show a view inspection report button, clicking the button will navigate the user to the Machinery Inspection Service Negotiation report dynamic route of the user dashboard.
## Admin Dashboard
...
3. The UI Of the inspections negotiations page will be similar to the ui of the listings page, where it will show the inspections card.
4. Clicking on the Machinery Inspection Service Negotiation negotiation will navigate the user to the Machinery Inspection Service Negotiation negotiation page.
5. The UI of Machinery Inspection Service Negotiation Service Negotiations page is also similar to Machinery Inspection Service Negotiation Services Lisitng page whcih will render all the negotiations of all the Users.
6. Clicking on the view button of a particular Machinery Inspection Service Negotiation Service negotiation will take the admin to the negotiation-detail/:Machinery Inspection Service Negotiation ServiceNegotiationID.
7. The UI of the Machinery Inspection Service Negotiation Service negotiation detail page will follow the UI of the admin dashboard where it will render all the information of the respective Machinery Inspection Service Negotiation Service negotiation.
8. In the Machinery Inspection Service Negotiation Service negotiation detail page, the admin can change the status from the dropdown of advanceStatus and finalPaymentStatus.
