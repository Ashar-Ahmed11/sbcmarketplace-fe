now we are going to create the mechanism for the Repair Service having the same mechanism as trucks negotiation which has been implemented.
# Repair Service Negotiation
## Backend
1. in the models folder create a new schema for Repair Service negotiation which consist of the following fields:
   1. buyer: id ref(user)
   2. seller: id ref(user)
   3. repairService: id ref(repairService)
   4. faultDescription: string
   5. faultImages: [{url:string}]
   6. onSite: boolean
   ...
   9. negotiation:[{
      ...
      partsResponsibility: string enum('buyer','seller')
...
2. in the routes folder, create the respective route file for repairServiceNegotiation to manage its crud endpoints.
## Frontend
## User Dashboard
...
3. clicking on the Repair Service negotiation will take the user to the spare-part-negotiations page whose ui is similar to Repair Services listing page.
In the Repair Services negotiation page it will fetch and show all the negotiations whose buyer or seller field matches the logged in user id.
4. Clicking on the view button will take the user to the /repair-service-negotiation/:repairServiceNegotiationID dynamic route which is described below in next point.

## Repair Service Details Page
1. [Repair ServiceDetailsSidebarCards.jsx](/Users/mac/gitRepos/sbc-marketplaced/Custom-ReactJS-Workflow/src/components/marketplace/Repair ServiceDetailsSidebarCards.jsx) write Offer Repair Service Amoount instead of Offer Amount.
2. remove the message textarea in the submit your offer card.
3. add a dropdown for parts responsibility (buyer or seller)
4. text area input for faultDescription
5. Fault Images: where user can upload images.
   Make sure to use the same upload Upload Image, Remove Images and its preview mechanism.
6. In the select delivery type if the user selects On-Site then below the buttons, 1 city dropdown, 1 text input for Buyer Delivery City and Buyer Delivery Address (as per the Repair Service negotiation schema) should become visible.
7. If Workshop is selected then no need to show the dropdown and address input and delivery cost input. (makes the onsite field false as per the schema)
8. when clicking on submit offer button then it should open a bootstrap confirmation modal saying that are you sure you want to submit an offer etc. 
   1. When the user clicks on submit then it should create a Repair Service negotiation with the provided information.
   2. make sure the UI of the confirmation modal matches the UI and theme of the website.
   3. after clicking the submit button the Repair Service negotiation document should be created as follow:
      1. the create Repair Service negotiation endpoint uses the authtoken middleware by which we can get the user id and use it in the buyer field.
      2. the seller id comes from the respective Repair Service listing document.
         1. from the frontend the Repair Serviceid is sent in the req.body, so inside the endpoint using the Repair Serviceid you retrieve the Repair Service document from whcih you can retrieve the sellerid.
      3. the Repair Serviceid is sent in the req.body from the frontend
      ...
      6. at the time of Repair Service negotiation creation, in the negotiation array, 1 object will be added where
         1. negotiator will be 'buyer'
         2. partsResponsibility: buyer or seller as per the selected dropdown item.
         3. labourCharges will be 0
        
      7. the rest of the fields of the Repair Service negotiation document should be null/default
   4. when a negotiation is submitted then it should navigate the user to the Repair Service-negotiation/:Repair ServicenegotiationID dynamic route of the user dashboard.
   ...
   Here it will render all the negotiations of the negotiation array of the Repair Service negotiation object.
   ...
   11. In the Pay Advance Fee Section:
       ...
       2. Show the Repair Service Cost and Delivery Cost.
       3. Show the Amount of Advance Fee
          1. The Advance fee is calculated as follows:
             (labourCharges+deliveryCost)*advancePercentage
       ...
       9. In this final payment section it will show the following amounts:
          1. Agreed Repair Service Cost=labourCharges
          ...
          3. Agreed Total Cost = Repair Service+Delivery
       ...
       10. once the finalPaymentStatus is approved then it should update the purchaseOrderDate to the current date and it will show negotiation successful and a Purchase Order will be visible which consist of the following informations:
          1. The Strucutre of the Repair Service Purchase Order should be similar to the reference purchase order image ![reference purchase order](image.png).
         ...
         7. The item table should consist of the following columns:
            1. Title (render the Repair Service title)
            2. Category (render the category name of the Repair Service)
            5. Repair Service cost
...
2. The UI of the Negotiations page should be similar to the Ui of View listings, when the Admin clicks on the Repair Services Negotiation then it should navigate the admin to the Repair Service-neogtiations page.
3. The UI of Repair Service Negotiations page is also similar to Repair Services Lisitng page whcih will render all the negotiations of all the Users.
4. Clicking on the view button of a particular Repair Service negotiation will take the admin to the negotiation-detail/:Repair ServiceNegotiationID.
5. The UI of the Repair Service negotiation detail page will follow the UI of the admin dashboard where it will render all the information of the respective Repair Service negotiation.
6. In the Repair Service negotiation detail page, the admin can change the status from the dropdown of advanceStatus and finalPaymentStatus.
