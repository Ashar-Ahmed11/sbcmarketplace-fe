# Spare Part Inspection Service Negotiation Report
## Backend
1. 1. in the models folder create a new schema for Spare Part Inspection Service Negotiation Report negotiation which consist of the following fields:
   1. inspectionRequester: id ref(user)
   2. inspector: id ref(user)
   3. sparePartInspectionNegotiation: id ref (sparePartInspectionNegotiation)
   ...
   7. brandVerification: {verified: boolean}
   8. physicalCondition: {score:number
	                     images:[{url:string}]}
   9. packagingCondition:
	{isPackagingAvailable:boolean
	score
	images}


2. in the routes folder, create the respective route file for sparePartInspectionReport to manage its crud endpoints.
...
1. In the user dashboard add another tab of Inspection same as Listings, where it shows all the types of inspection cards which include:
   1. Truck Inspection
   2. Spare Part Inspection
   3. Spare Part Inspection
   4. Material Inspection
2. Clicking on the Spare Part Inspection Service will take the user to the Spare Part Inspection Service page, whose ui and other things will be similar to the truck listings page where it will show all the Spare Part Inspection Service Negotiations whose (inspectionRequester id matches the logged in user and the sparePartInspectionReport.sparePartInspectionNegotiation.finalPaymentStatus is paid) or inspector id matches the id of the logged in user.
   1. At the top right of the Spare Part Inspection Service Negotiation page, there will be a create report button which takes the user to the /create-truck-inspection-report page.
   The Create Spare Part Inspection Service Negotiation Report page ui and mechanism should be as follows:
   1. It shows all the sections and inputs as per the sparePartInspectionReport schema,
      1. at the start of the inspection form, there will be a search and select for select Spare Part Inspection Service Negotiation negotiation the same way as it is used for product type in [transaction.jsx](/Users/mac/gitRepos/sbc-marketplaced/Custom-ReactJS-Workflow/referenceRepo/referenceComponent/transaction.jsx).
         1. it fetches and shows those Spare Part Inspection Service Negotiation negotiations whose seller id matches the id of the logged in user and the advance status is paid.
         and the sparePartInspectionReport id is null
         2. it shows the results whose buyer.fullname or buyer.username matches the search query. (it should be broad search)
      2. selecting a negotiation in the select Spare Part Inspection Service Negotiation negotiation will use the buyer._id for the inspectionRequester field and use the selected negotiation id for the sparePartInspectionNegotiation field from the selected negotiation.
   ...
   4. Clicking on submit button will submit the Spare Part Inspection Service Negotiation report and also adds & update the id of the newly created Spare Part Inspection Service Negotiation report to the selected Spare Part Inspection Service Negotiation negotiation document.
3. In the Spare Part Inspection Service Negotiation page, if a user selects a Spare Part Inspection Service Negotiation then if the logged in user is a inspectionRequester then only the report is visible to him.
...
1. In the admin dashboard add another tab of Inspection same as Listings, where it shows all the types of inspection cards which include:
   ...
   3. Spare Part Inspection
   4. Material Inspection
2. Clicking on the Spare Part Inspection Service Negotiation will take the user to the Spare Part Inspection Service Negotiation page, whose ui and other things will be similar to the truck listings page where it will show all the Spare Part Inspection Service Negotiations.
...
now we are going to create the mechanism for the Spare Part Inspection Service Negotiation Service having the same mechanism as trucks negotiation which has been implemented.
# Spare Part Inspection Service Negotiation Service Negotiation
## Backend
1. in the models folder create a new schema for Spare Part Inspection Service Negotiation Service negotiation which consist of the following fields:
   1. buyer: id ref(user)
   ...
   14. sparePartInspectionReport: id ref(sparePartInspectionReport)
   15. purhcaseOrderDate: Date
2. in the routes folder, create the respective route file for sparePartInspectionNegotiation to manage its crud endpoints.
## Frontend
## User Dashboard
...
4. Clicking on the Spare Part Inspection Service Negotiation negotiation will navigate the user to the Spare Part Inspection Service Negotiation negotiation page.
5. The UI of Spare Part Inspection Service Negotiation Service Negotiations page is also similar to Spare Part Inspection Service Negotiation Services Lisitng page.
In the Spare Part Inspection Service Negotiation Services negotiation page it will fetch and show all the negotiations whose buyer or seller field matches the logged in user id.
4. Clicking on the view button will take the user to the /inspection-service-negotiation/:sparePartInspectionNegotiationID dynamic route which is described below in next point.

## Inspection Service Details Page
1. [ Inspection ServiceDetailsSidebarCards.jsx](/Users/mac/gitRepos/sbc-marketplaced/Custom-ReactJS-Workflow/src/components/marketplace/Spare Part Inspection Service Negotiation ServiceDetailsSidebarCards.jsx) write Offer Spare Part Inspection Service Negotiation Service Amoount instead of Offer Amount.
...
If the selected product category.categoryType is spareParts then negotiation for the sparePartInspectionNegotiation will be created when clicking on submit request.
...
4. In the select delivery type if the user selects On-Site then below the buttons, 1 city dropdown, 1 text input for Buyer Delivery City and Buyer Delivery Address (as per the Spare Part Inspection Service Negotiation Service negotiation schema) should become visible.
...
6. when clicking on submit offer button then it should open a bootstrap confirmation modal saying that are you sure you want to submit an offer etc. 
   1. When the user clicks on submit then it should create a Spare Part Inspection Service Negotiation Service negotiation with the provided information.
   2. make sure the UI of the confirmation modal matches the UI and theme of the website.
   3. after clicking the submit button the Spare Part Inspection Service Negotiation Service negotiation document should be created as follow:
      1. the create Spare Part Inspection Service Negotiation Service negotiation endpoint uses the authtoken middleware by which we can get the user id and use it in the buyer field.
      2. the seller id comes from the respective Spare Part Inspection Service Negotiation Service listing document.
         1. from the frontend the Spare Part Inspection Service Negotiation Serviceid is sent in the req.body, so inside the endpoint using the Spare Part Inspection Service Negotiation Serviceid you retrieve the Spare Part Inspection Service Negotiation Service document from whcih you can retrieve the sellerid.
      3. the Spare Part Inspection Service Negotiation Serviceid is sent in the req.body from the frontend
      ...
      6. at the time of Spare Part Inspection Service Negotiation Service negotiation creation, in the negotiation array, 1 object will be added where
         1. negotiator will be 'buyer'
         2. labourCharges will be 0
         
      7. the rest of the fields of the Spare Part Inspection Service Negotiation Service negotiation document should be null/default
   4. when a negotiation is submitted then it should navigate the user to the Spare Part Inspection Service Negotiation Service-negotiation/:Spare Part Inspection Service Negotiation ServicenegotiationID dynamic route of the user dashboard.
   ...
   Here it will render all the negotiations of the negotiation array of the Spare Part Inspection Service Negotiation Service negotiation object.
   ...
   11. In the Pay Advance Fee Section:
       ...
       2. Show the Spare Part Inspection Service Negotiation Service Cost and Delivery Cost.
       ...
       8. once the status has been changed to paid by the admin then this section will now become invisible just like negotiations and now the Spare Part Inspection Service Negotiation report status section will be visible which shows the status pending if there is no id assigned to the sparePartInspectionReport field of the sparePartInspectionNegotiation.

9. when the id to the sparePartInspectionReport is assigned then the status will be changed to pending approval following the status of the sparePartInspectionNegotiation.sparePartInspectionReport.status
       
10. when the sparePartInspectionReport.status is changed to approved then the Final Payment section will be visible following the UI of the figma design: [SBC Marketplace UI 1.0 Copy](https://www.figma.com/design/uJ82aUAo9qF9dUIBSw4lNP/SBC-Marketplace-UI-1.0--Copy-?node-id=253-3171&t=Q68W0nWhC32fdKID-4).
...
9. In this final payment section it will show the following amounts:
   1. Agreed Spare Part Inspection Service Negotiation Service Cost=labourCharges
   2. Agreed Total Cost = Spare Part Inspection Service Negotiation Service+Delivery
...
10. once the finalPaymentStatus is approved then it should update the purchaseOrderDate to the current date and it will show negotiation successful and a Purchase Order will be visible which consist of the following informations:
    1. The Strucutre of the Spare Part Inspection Service Negotiation Service Purchase Order should be similar to the reference purchase order image ![reference purchase order](image.png).
    ...
    7. The item table should consist of the following columns:
       1. Title (render the Spare Part Inspection Service Negotiation Service title)
       2. Category (render the category name of the Spare Part Inspection Service Negotiation Service)
       3. Spare Part Inspection Service Negotiation Service cost
...
11. before the purchase order show a view inspection report button, clicking the button will navigate the user to the Spare Part Inspection Service Negotiation report dynamic route of the user dashboard.
## Admin Dashboard
...
3. The UI Of the inspections negotiations page will be similar to the ui of the listings page, where it will show the inspections card.
4. Clicking on the Spare Part Inspection Service Negotiation negotiation will navigate the user to the Spare Part Inspection Service Negotiation negotiation page.
5. The UI of Spare Part Inspection Service Negotiation Service Negotiations page is also similar to Spare Part Inspection Service Negotiation Services Lisitng page whcih will render all the negotiations of all the Users.
6. Clicking on the view button of a particular Spare Part Inspection Service Negotiation Service negotiation will take the admin to the negotiation-detail/:Spare Part Inspection Service Negotiation ServiceNegotiationID.
7. The UI of the Spare Part Inspection Service Negotiation Service negotiation detail page will follow the UI of the admin dashboard where it will render all the information of the respective Spare Part Inspection Service Negotiation Service negotiation.
8. In the Spare Part Inspection Service Negotiation Service negotiation detail page, the admin can change the status from the dropdown of advanceStatus and finalPaymentStatus.
