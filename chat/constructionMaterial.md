now we are going to create the mechanism for the Construction Material having the same mechanism as trucks negotiation which has been implemented.
# Construction Material Negotiation
## Backend
1. in the models folder create a new schema for Construction Material negotiation which consist of the following fields:
   1. buyer: id ref(user)
   2. seller: id ref(user)
   3. constructionMaterial: id ref(constructionMaterial)
   ...
   8. negotiation:[{
      ...
      constructionMaterialCost:number
      deliveryCost:number
      accepted:boolean default false
      }]
   9. advanceStatus:string enum('unpaid', 'pendingApproval','paid') default 'unpaid'
   10. advanceStatusRejectionReason: string
   11. advancePaymentScreenshots:[{url:string}]
   12. finalPaymentStatus:string enum('unpaid', 'pendingApproval','paid') default 'unpaid'
   13. finalPaymentStatusRejectionReason:string
   14. finalPaymentScreenshots:[{url:string}]
   15. purhcaseOrderDate: Date
2. in the routes folder, create the respective route file for constructionMaterialNegotiation to manage its crud endpoints.
## Frontend
## User Dashboard
...
3. clicking on the Construction Material negotiation will take the user to the spare-part-negotiations page whose ui is similar to Construction Materials listing page.
In the Construction Materials negotiation page it will fetch and show all the negotiations whose buyer or seller field matches the logged in user id.
...
## Construction Material Details Page
1. [Construction MaterialDetailsSidebarCards.jsx](/Users/mac/gitRepos/sbc-marketplaced/Custom-ReactJS-Workflow/src/components/marketplace/Construction MaterialDetailsSidebarCards.jsx) write Offer Construction Material Amoount instead of Offer Amount.
2. remove the message textarea in the submit your offer card.
2. After the construction material offer input, add another input for construction material quantity 
3. In the select delivery type if the user selects Seller delivery then below the buttons, 1 city dropdown, 1 text input, 1 number input for Buyer Delivery City and Buyer Delivery Address and delivery cost (as per the Construction Material negotiation schema) should become visible.
...
5. when clicking on submit offer button then it should open a bootstrap confirmation modal saying that are you sure you want to submit an offer etc. 
   1. When the user clicks on submit then it should create a Construction Material negotiation with the provided information.
   2. make sure the UI of the confirmation modal matches the UI and theme of the website.
   3. after clicking the submit button the Construction Material negotiation document should be created as follow:
      1. the create Construction Material negotiation endpoint uses the authtoken middleware by which we can get the user id and use it in the buyer field.
      2. the seller id comes from the respective Construction Material listing document.
         1. from the frontend the Construction Materialid is sent in the req.body, so inside the endpoint using the Construction Materialid you retrieve the Construction Material document from whcih you can retrieve the sellerid.
      3. the Construction Materialid is sent in the req.body from the frontend
      ...
      6. at the time of Construction Material negotiation creation, in the negotiation array, 1 object will be added where
         1. negotiator will be 'buyer'
         2. constructionMaterialCost will be the amount that is taken from the Offer Construction Material Amount from the offer form.
      ...
      7. quantity will be the amount that is taken from that is taken from the constructuon material quantity input.
      7. the rest of the fields of the Construction Material negotiation document should be null/default
   4. when a negotiation is submitted then it should navigate the user to the Construction Material-negotiation/:Construction MaterialnegotiationID dynamic route of the user dashboard.
   ...
   Here it will render all the negotiations of the negotiation array of the Construction Material negotiation object.
   ...
   7. Clicking the counter offer button will open a bootstrap model for Submit Cuounter offer, which consists of the following inputs:
      1. Construction Material Cost Number input
   ...
   11. In the Pay Advance Fee Section:
       ...
       2. Show the Construction Material Cost and Delivery Cost.
       3. Show the Amount of Advance Fee
          1. The Advance fee is calculated as follows:
             (constructionMaterialCost+deliveryCost)*advancePercentage
       ...
       9. In this final payment section it will show the following amounts:
          1. Agreed Construction Material Cost=constructionMaterialCost
          ...
          3. Agreed Total Cost = Construction Material+Delivery
       ...
       10. once the finalPaymentStatus is approved then it should update the purchaseOrderDate to the current date and it will show negotiation successful and a Purchase Order will be visible which consist of the following informations:
          1. The Strucutre of the Construction Material Purchase Order should be similar to the reference purchase order image ![reference purchase order](image.png).
         ...
         7. The item table should consist of the following columns:
            1. Title (render the Construction Material title)
            2. Category (render the category name of the Construction Material)
            3. Construction Material Brand
            4. Construction Material Quantity
            5. Construction Material cost
            In the last(total column) and second last column of the table show the following breakdown:
             1. Subtotal=constructionMaterialCost
         2. Delivery Cost = deliveryCost
          3. Advance Fee Amount = (constructionMaterialCost+deliveryCost)*advancePercentage
          4. Platform Fee = (constructionMaterialCost+deliveryCost-advance fee amount)*platformFeePercentage
          5. Total= (constructionMaterialCost+deliveryCost) - advance fee amount + platform fee
...
2. The UI of the Negotiations page should be similar to the Ui of View listings, when the Admin clicks on the Construction Materials Negotiation then it should navigate the admin to the Construction Material-neogtiations page.
3. The UI of Construction Material Negotiations page is also similar to Construction Materials Lisitng page whcih will render all the negotiations of all the Users.
4. Clicking on the view button of a particular Construction Material negotiation will take the admin to the negotiation-detail/:Construction MaterialNegotiationID.
5. The UI of the Construction Material negotiation detail page will follow the UI of the admin dashboard where it will render all the information of the respective Construction Material negotiation.
6. In the Construction Material negotiation detail page, the admin can change the status from the dropdown of advanceStatus and finalPaymentStatus.
