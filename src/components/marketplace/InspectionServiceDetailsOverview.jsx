...
import InspectionServiceDetailsSidebarCards from './InspectionServiceDetailsSidebarCards';
import ServiceSpecialistDetailsInfoSections from './ServiceSpecialistDetailsInfoSections';
import ServiceSpecialistDetailsSpecsCard from './ServiceSpecialistDetailsSpecsCard';
...
function InspectionServiceDetailsOverview({ inspectionService }) {
  const galleryItems = [...(inspectionService?.images || [])].filter((item) => item?.url);
      ...
      <div className="container-xl">
        <div className="truck-figma-main-grid">
          <div className="truck-figma-left-column">
                    ...
                    {galleryItems.map((asset, index) => (
                      <div className="keen-slider__slide number-slide" key={`${asset.url}-${index}`}>
                        <div className="truck-figma-gallery-frame">
                          ...
                          <img alt={`${inspectionService?.title || 'Inspection service'} ${index + 1}`} onLoad={() => setImageLoaded(true)} src={asset.url} />
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div ref={thumbnailRef} className="keen-slider truck-figma-thumbnail mt-3">
                    {galleryItems.map((asset, index) => (
                      <div className="keen-slider__slide number-slide d-flex align-items-center" key={`${asset.url}-thumb-${index}`}>
                        <img alt={`${inspectionService?.title || 'Inspection service'} thumbnail ${index + 1}`} className="w-100" src={asset.url} />
                      </div>
                    ))}
                  </div>
                ) : null}
            ...
            <ServiceSpecialistDetailsSpecsCard service={inspectionService} titleFallback="Inspection Service Listing" />
            <ServiceSpecialistDetailsInfoSections areaField="inspectionAreas" areaLabel="Inspection Areas" service={inspectionService} />
          </div>
          <InspectionServiceDetailsSidebarCards inspectionService={inspectionService} />
        </div>
      </div>
    </section>
  );
...
export default InspectionServiceDetailsOverview;
