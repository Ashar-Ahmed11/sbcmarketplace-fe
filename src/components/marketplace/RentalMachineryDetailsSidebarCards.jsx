...
function RentalMachineryDetailsSidebarCards({ rentalMachinery }) {
  ...
  const { createRentalConstructionMachineryNegotiation, currentUser, userToken } = useContext(AppContext);
  ...
  const availableDeliveryCities = useMemo(
    () => (rentalMachinery?.deliveryLocations || []).filter((item) => item?.city),
    [rentalMachinery?.deliveryLocations]
  );
  ...
  const summaryText = useMemo(() => {
    const deliveryText = offer.sellerDelivery
      ...
      : 'SBC delivery';
    return `Submit a rental offer of Rs. ${offer.perDayRentalCharges || 0} per day for ${rentalMachinery?.title || 'this rental construction machinery'} with ${deliveryText}?`;
  }, [offer.buyerDeliveryCity, offer.perDayRentalCharges, offer.sellerDelivery, rentalMachinery?.title]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const created = await createRentalConstructionMachineryNegotiation({
        rentalConstructionMachineryId: rentalMachinery._id,
      ...
      history.push(`/user-dashboard/rental-construction-machinery-negotiation/${created._id}`);
    } finally {
      setIsSubmitting(false);
    }
  };

  const isOwner = String(currentUser?._id) === String(rentalMachinery?.user?._id);
        ...
        <div className="truck-figma-field">
          ...
          <div className="truck-figma-delivery-options">
            <button
              ...
              disabled={!rentalMachinery?.deliveryProvided}
              onClick={() => rentalMachinery?.deliveryProvided && setOffer((current) => ({ ...current, sellerDelivery: true, deliveryCost: current.deliveryCost || String(selectedDeliveryLocation?.price || '') }))}
              type="button"
            >
              Seller Delivery
            </button>
