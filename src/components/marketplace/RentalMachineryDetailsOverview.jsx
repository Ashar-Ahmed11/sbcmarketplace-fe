...
import RentalMachineryDetailsInfoSections from './RentalMachineryDetailsInfoSections';
import RentalMachineryDetailsSidebarCards from './RentalMachineryDetailsSidebarCards';
import RentalMachineryDetailsSpecsCard from './RentalMachineryDetailsSpecsCard';
...
function RentalMachineryDetailsOverview({ rentalMachinery }) {
  const galleryItems = [...(rentalMachinery?.images || [])].filter((item) => item?.url);
      ...
      <div className="container-xl">
        <div className="truck-figma-main-grid">
          <div className="truck-figma-left-column">
                    ...
                    {galleryItems.map((asset, index) => (
                      <div className="keen-slider__slide number-slide" key={`${asset.url}-${index}`}>
                        <div className="truck-figma-gallery-frame">
                          ...
                          <img alt={`${rentalMachinery?.title || 'Rental machinery'} ${index + 1}`} onLoad={() => setImageLoaded(true)} src={asset.url} />
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div ref={thumbnailRef} className="keen-slider truck-figma-thumbnail mt-3">
                    {galleryItems.map((asset, index) => (
                      <div className="keen-slider__slide number-slide d-flex align-items-center" key={`${asset.url}-thumb-${index}`}>
                        <img alt={`${rentalMachinery?.title || 'Rental machinery'} thumbnail ${index + 1}`} className="w-100" src={asset.url} />
                      </div>
                    ))}
                  </div>
                ) : null}
            ...
            <RentalMachineryDetailsSpecsCard rentalMachinery={rentalMachinery} />
            <RentalMachineryDetailsInfoSections rentalMachinery={rentalMachinery} />
          </div>
          <RentalMachineryDetailsSidebarCards rentalMachinery={rentalMachinery} />
        </div>
      </div>
    </section>
  );
...
export default RentalMachineryDetailsOverview;
