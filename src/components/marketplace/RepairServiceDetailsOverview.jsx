...
import RepairServiceDetailsSidebarCards from './RepairServiceDetailsSidebarCards';
...
function RepairServiceDetailsOverview({ repairService }) {
  const galleryItems = [...(repairService?.images || [])].filter((item) => item?.url);
      ...
      <div className="container-xl">
        <div className="truck-figma-main-grid">
          <div className="truck-figma-left-column">
                    ...
                    {galleryItems.map((asset, index) => (
                      <div className="keen-slider__slide number-slide" key={`${asset.url}-${index}`}>
                        <div className="truck-figma-gallery-frame">
                          ...
                          <img alt={`${repairService?.title || 'Repair service'} ${index + 1}`} onLoad={() => setImageLoaded(true)} src={asset.url} />
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div ref={thumbnailRef} className="keen-slider truck-figma-thumbnail mt-3">
                    {galleryItems.map((asset, index) => (
                      <div className="keen-slider__slide number-slide d-flex align-items-center" key={`${asset.url}-thumb-${index}`}>
                        <img alt={`${repairService?.title || 'Repair service'} thumbnail ${index + 1}`} className="w-100" src={asset.url} />
                      </div>
                    ))}
                  </div>
                ) : null}
            ...
            <ServiceSpecialistDetailsSpecsCard service={repairService} titleFallback="Repair Service Listing" />
            <ServiceSpecialistDetailsInfoSections areaField="repairAreas" areaLabel="Repair Areas" service={repairService} />
          </div>
          <RepairServiceDetailsSidebarCards repairService={repairService} />
        </div>
      </div>
    </section>
  );
...
export default RepairServiceDetailsOverview;
