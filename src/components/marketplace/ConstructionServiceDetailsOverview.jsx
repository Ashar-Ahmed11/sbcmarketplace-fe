...
import ConstructionServiceDetailsInfoSections from './ConstructionServiceDetailsInfoSections';
import ConstructionServiceDetailsSidebarCards from './ConstructionServiceDetailsSidebarCards';
import ConstructionServiceDetailsSpecsCard from './ConstructionServiceDetailsSpecsCard';
...
function ConstructionServiceDetailsOverview({ constructionService }) {
  const galleryItems = [...(constructionService?.images || [])].filter((item) => item?.url);
      ...
      <div className="container-xl">
        <div className="truck-figma-main-grid">
          <div className="truck-figma-left-column">
                    ...
                    {galleryItems.map((asset, index) => (
                      <div className="keen-slider__slide number-slide" key={`${asset.url}-${index}`}>
                        <div className="truck-figma-gallery-frame">
                          ...
                          <img alt={`${constructionService?.title || 'Construction service'} ${index + 1}`} onLoad={() => setImageLoaded(true)} src={asset.url} />
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div ref={thumbnailRef} className="keen-slider truck-figma-thumbnail mt-3">
                    {galleryItems.map((asset, index) => (
                      <div className="keen-slider__slide number-slide d-flex align-items-center" key={`${asset.url}-thumb-${index}`}>
                        <img alt={`${constructionService?.title || 'Construction service'} thumbnail ${index + 1}`} className="w-100" src={asset.url} />
                      </div>
                    ))}
                  </div>
                ) : null}
            ...
            <ConstructionServiceDetailsSpecsCard constructionService={constructionService} />
            <ConstructionServiceDetailsInfoSections constructionService={constructionService} />
          </div>
          <ConstructionServiceDetailsSidebarCards constructionService={constructionService} />
        </div>
      </div>
    </section>
  );
...
export default ConstructionServiceDetailsOverview;
