...
function MachineryNegotiationTable({ rows, title, subtitle, viewBasePath }) {
  return (
        ...
        <table className="table dashboard-table">
          ...
          <tbody>
            {rows.length ? rows.map((item) => {
              ...
              return (
                <tr key={item._id}>
                  <td className="py-3">{item.constructionMachinery?.title || 'Machinery Negotiation'}</td>
                  ...
                  <td className="py-3">{latest?.machineryCost ? `Rs. ${Number(latest.machineryCost).toLocaleString()}` : '—'}</td>
              ...
              <tr><td className="text-center py-4" colSpan="7">No construction machinery negotiations found.</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
export default MachineryNegotiationTable;
