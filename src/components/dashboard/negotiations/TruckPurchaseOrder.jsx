...
import { getAcceptedOffer, getNegotiationTotals } from './truckNegotiationUtils';

function formatCurrency(value) {
  return `Rs. ${Number(value || 0).toLocaleString()}`;
}

function TruckPurchaseOrder({ basicInfo, row }) {
        ...
        <table className="table truck-purchase-order__table">
          ...
          <tbody>
            <tr>
              ...
              <td>{formatCurrency(totals.truckCost)}</td>
              <td>{formatCurrency(totals.truckCost)}</td>
            </tr>
            <tr>
              <td colSpan="3" />
...
export default TruckPurchaseOrder;
