...
import { getTruckInspectionOverallScore } from './truckInspectionReports/truckInspectionReportUtils';

function TruckInspectionReportTable({ actionLabel, actionTo, rows, subtitle, title, viewBasePath }) {
  return (
        ...
        <table className="table dashboard-table">
          ...
          <tbody>
            {rows.length ? rows.map((item) => (
              <tr key={item._id}>
                <td className="py-3">{item.truckInspectionServiceNegotiation?.truck?.title || 'Truck Inspection Report'}</td>
                ...
                <td className="py-3">{getTruckInspectionOverallScore(item)}%</td>
              ...
              <tr><td className="text-center py-4" colSpan="7">No truck inspection reports found.</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
export default TruckInspectionReportTable;
