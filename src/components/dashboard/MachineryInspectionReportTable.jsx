...
import { getMachineryInspectionOverallScore } from './machineryInspectionReports/machineryInspectionReportUtils';

function MachineryInspectionReportTable({ actionLabel, actionTo, rows, subtitle, title, viewBasePath }) {
  return (
        ...
        <table className="table dashboard-table">
          ...
          <tbody>
            {rows.length ? rows.map((item) => (
              <tr key={item._id}>
                <td className="py-3">{item.machineryInspectionNegotiation?.constructionMachinery?.title || 'Machinery Inspection Report'}</td>
                ...
                <td className="py-3">{getMachineryInspectionOverallScore(item)}%</td>
              ...
              <tr><td className="text-center py-4" colSpan="7">No machinery inspection reports found.</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
export default MachineryInspectionReportTable;
