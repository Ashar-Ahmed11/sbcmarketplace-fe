...
function AdminConstructionMaterialInspectionNegotiationDetailPage() {
  ...
  }, [constructionMaterialInspectionNegotiationId, getBasicInfo, getConstructionMaterialInspectionNegotiationById]);
  ...
  return (
    <section className="dashboard-section-card form-card-panel">
      <div className="dashboard-section-head"><div><h1>Negotiation Detail</h1><p>{row.inspectionService?.title || 'Construction material inspection service negotiation'}</p></div></div>
      <div className="truck-figma-details-stack mb-4">
        <section className="truck-figma-specs-card truck-figma-specs-card--compact">
          ...
          <div className="truck-figma-specs-table">
            ...
            <div className="truck-figma-specs-row alt">
              <div className="truck-figma-specs-cell"><span>Material</span><strong>{row.constructionMaterial?.title || '—'}</strong></div>
        ...
        <section className="truck-figma-specs-card truck-figma-specs-card--compact">
          <div className="dashboard-section-head mb-0"><div><h3>Conversation</h3></div><button className="dashboard-action-btn" data-bs-target="#adminConstructionMaterialInspectionNegotiationConversation" data-bs-toggle="collapse" type="button">View Conversation</button></div>
          <div className="collapse mt-3" id="adminConstructionMaterialInspectionNegotiationConversation">
            <ConstructionMaterialInspectionNegotiationConversation currentUserId={row.seller?._id} onAccept={() => {}} onOpenCounterOffer={() => {}} row={row} showActionButtons={false} showCounterButton={false} title="Construction Material Inspection Negotiation Conversation" />
          </div>
        </section>
        ...
        {row.advancePaymentScreenshots?.length ? (
          <section className="truck-figma-specs-card truck-figma-specs-card--compact">
            ...
            <div className="upload-preview-grid">{row.advancePaymentScreenshots.map((image, index) => <div className="upload-preview-card readonly" key={`${image.url}-${index}`}><img alt="Advance payment proof" src={image.url} /></div>)}</div>
        ...
        {row.finalPaymentScreenshots?.length ? (
          <section className="truck-figma-specs-card truck-figma-specs-card--compact">
            ...
            <div className="upload-preview-grid">{row.finalPaymentScreenshots.map((image, index) => <div className="upload-preview-card readonly" key={`${image.url}-${index}`}><img alt="Final payment proof" src={image.url} /></div>)}</div>
            <div className="dashboard-form-grid mt-3">
        ...
        {acceptedOffer && row.finalPaymentStatus === 'paid' ? <ConstructionMaterialInspectionPurchaseOrder basicInfo={basicInfo} reportBasePath="/admin-dashboard/construction-material-inspection-report" row={row} /> : null}
      </div>

      <div className="dashboard-form-actions">
        <button className="dashboard-action-btn" onClick={async () => setRow(await updateConstructionMaterialInspectionNegotiationStatus(constructionMaterialInspectionNegotiationId, {
...
export default AdminConstructionMaterialInspectionNegotiationDetailPage;
