...
function AdminSparePartNegotiationDetailPage() {
  const { sparePartNegotiationId } = useParams();
  const { basicInfo, getBasicInfo, getSparePartNegotiationById, updateSparePartNegotiationStatus } = useContext(AppContext);
  const [row, setRow] = useState(null);

  useEffect(() => {
    getBasicInfo();
    getSparePartNegotiationById(sparePartNegotiationId).then(setRow);
  }, [getBasicInfo, getSparePartNegotiationById, sparePartNegotiationId]);
    ...
    <section className="dashboard-section-card form-card-panel">
      <div className="dashboard-section-head">
        <div>
          ...
          <p>{row.sparePart?.title || 'Spare part negotiation'}</p>
        </div>
      </div>
      ...
      <div className="truck-figma-details-stack mb-4">
        ...
        <section className="truck-figma-specs-card truck-figma-specs-card--compact">
          <div className="dashboard-section-head mb-0">
            ...
            <button className="dashboard-action-btn" data-bs-target="#adminSparePartNegotiationConversation" data-bs-toggle="collapse" type="button">View Conversation</button>
          </div>
          <div className="collapse mt-3" id="adminSparePartNegotiationConversation">
            <SparePartNegotiationConversation
              ...
              title="Spare Part Negotiation Conversation"
            />
          </div>
        </section>
        ...
        {acceptedOffer && row.finalPaymentStatus === 'paid' ? <SparePartPurchaseOrder basicInfo={basicInfo} row={row} /> : null}
      </div>

      <div className="dashboard-form-actions">
        <button className="dashboard-action-btn" onClick={async () => setRow(await updateSparePartNegotiationStatus(sparePartNegotiationId, {
...
export default AdminSparePartNegotiationDetailPage;
