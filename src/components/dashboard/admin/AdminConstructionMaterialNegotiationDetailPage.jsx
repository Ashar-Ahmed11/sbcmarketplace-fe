...
function AdminConstructionMaterialNegotiationDetailPage() {
  const { materialNegotiationId } = useParams();
  const { basicInfo, getBasicInfo, getConstructionMaterialNegotiationById, updateConstructionMaterialNegotiationStatus } = useContext(AppContext);
  const [row, setRow] = useState(null);

  useEffect(() => {
    getBasicInfo();
    getConstructionMaterialNegotiationById(materialNegotiationId).then(setRow);
  }, [getBasicInfo, getConstructionMaterialNegotiationById, materialNegotiationId]);
    ...
    <section className="dashboard-section-card form-card-panel">
      <div className="dashboard-section-head">
        <div>
          ...
          <p>{row.constructionMaterial?.title || 'Construction material negotiation'}</p>
        </div>
      </div>
      ...
      <div className="truck-figma-details-stack mb-4">
        <section className="truck-figma-specs-card truck-figma-specs-card--compact">
          ...
          <div className="truck-figma-specs-table">
            ...
            <div className="truck-figma-specs-row alt">
              ...
              <div className="truck-figma-specs-cell"><span>Quantity</span><strong>{row.quantity ? `${row.quantity} ${row.constructionMaterial?.unit || ''}`.trim() : '—'}</strong></div>
            </div>
            <div className="truck-figma-specs-row">
            ...
            <div className="truck-figma-specs-row alt full">
              <div className="truck-figma-specs-cell">
                ...
                <strong>{row.buyerDeliveryAddress || '—'}</strong>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="truck-figma-specs-card truck-figma-specs-card--compact">
          <div className="dashboard-section-head mb-0">
            ...
            <button className="dashboard-action-btn" data-bs-target="#adminConstructionMaterialNegotiationConversation" data-bs-toggle="collapse" type="button">View Conversation</button>
          </div>
          <div className="collapse mt-3" id="adminConstructionMaterialNegotiationConversation">
            <ConstructionMaterialNegotiationConversation
              ...
              title="Construction Material Negotiation Conversation"
            />
          </div>
        </section>
        ...
        {acceptedOffer && row.finalPaymentStatus === 'paid' ? <ConstructionMaterialPurchaseOrder basicInfo={basicInfo} row={row} /> : null}
      </div>

      <div className="dashboard-form-actions">
        <button className="dashboard-action-btn" onClick={async () => setRow(await updateConstructionMaterialNegotiationStatus(materialNegotiationId, {
...
export default AdminConstructionMaterialNegotiationDetailPage;
