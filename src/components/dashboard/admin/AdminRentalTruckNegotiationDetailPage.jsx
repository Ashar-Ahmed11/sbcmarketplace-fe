...
function AdminRentalTruckNegotiationDetailPage() {
  const { rentalTruckNegotiationId } = useParams();
  const { basicInfo, getBasicInfo, getRentalTruckNegotiationById, updateRentalTruckNegotiationStatus } = useContext(AppContext);
  const [row, setRow] = useState(null);

  useEffect(() => {
    getBasicInfo();
    getRentalTruckNegotiationById(rentalTruckNegotiationId).then(setRow);
  }, [getBasicInfo, getRentalTruckNegotiationById, rentalTruckNegotiationId]);
      ...
      <div className="truck-figma-details-stack mb-4">
        ...
        <section className="truck-figma-specs-card truck-figma-specs-card--compact">
          <div className="dashboard-section-head mb-0">
            ...
            <button className="dashboard-action-btn" data-bs-target="#adminRentalTruckNegotiationConversation" data-bs-toggle="collapse" type="button">View Conversation</button>
          </div>
          <div className="collapse mt-3" id="adminRentalTruckNegotiationConversation">
            <RentalTruckNegotiationConversation
              ...
              title="Rental Truck Negotiation Conversation"
            />
          </div>
        </section>
        ...
        {acceptedOffer && row.finalPaymentStatus === 'paid' ? <RentalTruckPurchaseOrder basicInfo={basicInfo} row={row} /> : null}
      </div>

      <div className="dashboard-form-actions">
        <button className="dashboard-action-btn" onClick={async () => setRow(await updateRentalTruckNegotiationStatus(rentalTruckNegotiationId, {
...
export default AdminRentalTruckNegotiationDetailPage;
