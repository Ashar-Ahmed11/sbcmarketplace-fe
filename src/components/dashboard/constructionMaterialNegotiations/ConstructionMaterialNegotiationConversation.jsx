import { getAcceptedConstructionMaterialOffer } from './constructionMaterialNegotiationUtils';

function formatCurrency(value) {
  return `Rs. ${Number(value || 0).toLocaleString()}`;
}

function ConstructionMaterialNegotiationConversation({
  currentUserId,
  onAccept,
  onOpenCounterOffer,
  row,
          ...
          return (
            <div className={`truck-negotiation-bubble ${isViewer ? 'buyer' : 'seller'}`} key={item._id}>
              <div className="truck-negotiation-bubble__inner">
                ...
                <strong>
                  {formatCurrency(item.constructionMaterialCost)}
                  {item.deliveryCost !== null && item.deliveryCost !== undefined ? ` + ${formatCurrency(item.deliveryCost)} delivery` : ''}
...
export default ConstructionMaterialNegotiationConversation;
