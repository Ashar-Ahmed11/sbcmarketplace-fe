...
import { getAcceptedConstructionMaterialOffer, getConstructionMaterialNegotiationTotals } from './constructionMaterialNegotiationUtils';

function formatCurrency(value) {
  return `Rs. ${Number(value || 0).toLocaleString()}`;
}

function ConstructionMaterialPurchaseOrder({ basicInfo, row }) {
    ...
    <section className="dashboard-section-card truck-purchase-order">
      ...
      <div className="truck-purchase-order__table-wrap">
        <table className="table truck-purchase-order__table">
          ...
          <tbody>
            <tr>
              <td>{row.constructionMaterial?.title || '—'}</td>
              <td>{row.constructionMaterial?.category?.name || '—'}</td>
              <td>{row.constructionMaterial?.brand || '—'}</td>
              <td>{row.quantity ? `${row.quantity} ${row.constructionMaterial?.unit || ''}`.trim() : '—'}</td>
              <td>{formatCurrency(totals.constructionMaterialCost)}</td>
              <td>{formatCurrency(totals.constructionMaterialCost)}</td>
            </tr>
            <tr>
              <td colSpan="4" />
...
export default ConstructionMaterialPurchaseOrder;
