...
import { getAcceptedConstructionServiceOffer, getConstructionServiceNegotiationTotals } from './constructionServiceNegotiationUtils';

function formatCurrency(value) {
  return `Rs. ${Number(value || 0).toLocaleString()}`;
}

function ConstructionServicePurchaseOrder({ basicInfo, row }) {
        ...
        <table className="table truck-purchase-order__table">
          <thead>
            <tr>
              <th>Title</th>
              <th>Category</th>
              <th>Construction Service Cost</th>
              <th>Total</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td>{row.constructionService?.title || '—'}</td>
              <td>{row.constructionService?.category?.name || '—'}</td>
...
export default ConstructionServicePurchaseOrder;
