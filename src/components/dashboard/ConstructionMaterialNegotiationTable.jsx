...
function ConstructionMaterialNegotiationTable({ rows, title, subtitle, viewBasePath }) {
  return (
        ...
        <table className="table dashboard-table">
          ...
          <tbody>
            {rows.length ? rows.map((item) => {
              ...
              return (
                <tr key={item._id}>
                  <td className="py-3">{item.constructionMaterial?.title || 'Construction Material Negotiation'}</td>
                  ...
                  <td className="py-3">{item.quantity ? `${item.quantity} ${item.constructionMaterial?.unit || ''}`.trim() : '—'}</td>
                  <td className="py-3">{latest?.constructionMaterialCost ? `Rs. ${Number(latest.constructionMaterialCost).toLocaleString()}` : '—'}</td>
              ...
              <tr><td className="text-center py-4" colSpan="8">No construction material negotiations found.</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
export default ConstructionMaterialNegotiationTable;
