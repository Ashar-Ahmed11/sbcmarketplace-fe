...
import RentalTruckNegotiationConversation from '../rentalTruckNegotiations/RentalTruckNegotiationConversation';
import RentalTruckNegotiationPaymentPanels from '../rentalTruckNegotiations/RentalTruckNegotiationPaymentPanels';
import RentalTruckPurchaseOrder from '../rentalTruckNegotiations/RentalTruckPurchaseOrder';
import { getAcceptedRentalTruckOffer } from '../rentalTruckNegotiations/rentalTruckNegotiationUtils';

function UserRentalTruckNegotiationDetailPage() {
  const { rentalTruckNegotiationId } = useParams();
  const {
  ...
  useEffect(() => {
    fetchUser();
    getBasicInfo();
    getRentalTruckNegotiationById(rentalTruckNegotiationId).then(setRow);
  }, [fetchUser, getBasicInfo, getRentalTruckNegotiationById, rentalTruckNegotiationId]);
  ...
  return (
    <>
      {(!acceptedOffer || row.advanceStatus !== 'paid') ? (
        <RentalTruckNegotiationConversation
          currentUserId={currentUser?._id}
          onAccept={async (negotiationId) => setRow(await acceptRentalTruckOffer(rentalTruckNegotiationId, { negotiationId }))}
          onOpenCounterOffer={() => setShowCounterModal(true)}
          row={row}
          title="Rental Truck Negotiation"
        />
      ) : null}

      {acceptedOffer ? (
        <RentalTruckNegotiationPaymentPanels
      ...
      {row.finalPaymentStatus === 'paid' ? <RentalTruckPurchaseOrder basicInfo={basicInfo} row={row} /> : null}
      ...
      {showCounterModal ? (
        <>
          ...
          <div
            aria-hidden="false"
            aria-labelledby="rentalTruckCounterOfferModalLabel"
            ...
            <div className="modal-dialog modal-dialog-centered modal-dialog-scrollable rental-negotiation-modal" role="document">
              <div className="modal-content rental-negotiation-modal__content">
                <div className="modal-header rental-negotiation-modal__header">
                  <div>
                    <h2 className="modal-title" id="rentalTruckCounterOfferModalLabel">Submit Counter Offer</h2>
                ...
                <div className="modal-body rental-negotiation-modal__body">
                  <div className="row">
                    <div className="form-field col-md-6 col-12">
                      <label>Rental Truck Per Day Negotiation Cost</label>
                ...
                <div className="modal-footer rental-negotiation-modal__footer">
                  ...
                  <button
                    ...
                    onClick={async () => {
                      setRow(await addRentalTruckCounterOffer(rentalTruckNegotiationId, counterOffer));
                      setCounterOffer({
                        securityDepositAmount: '',
                        perDayRentalCharges: '',
                        deliveryCost: '',
      ...
      <PaymentProofModal
        ...
        onSubmit={async (images) => {
          const updated = proofModal.type === 'advance'
            ? await submitRentalTruckAdvanceProof(rentalTruckNegotiationId, { advancePaymentScreenshots: images })
            : await submitRentalTruckFinalProof(rentalTruckNegotiationId, { finalPaymentScreenshots: images });
          setRow(updated);
...
export default UserRentalTruckNegotiationDetailPage;
