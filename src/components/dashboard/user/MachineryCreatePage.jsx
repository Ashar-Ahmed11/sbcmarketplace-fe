...
function MachineryCreatePage({ editMode = false }) {
  const {
    categories,
    countryOptions,
    createMachinery,
  ...
  const [form, setForm] = useState(machineryInitialForm);

  const machineriesRoute = useMemo(
    () => (location.pathname.includes('/user-dashboard/my-listings') ? '/user-dashboard/my-listings/construction-machinery' : '/user-dashboard/construction-machinery'),
    [location.pathname]
  );

  useEffect(() => {
    setForm(machineryInitialForm);
  }, [machineryInitialForm]);
  ...
  useEffect(() => {
    if (editMode && machineryId) {
      getMachineryById(machineryId).then((data) => {
        setForm((current) => ({
          ...current,
          ...data,
  ...
  }, [editMode, getMachineryById, machineryId]);
  ...
  const onSubmit = async () => {
    if (editMode && machineryId) {
      await updateMachinery(machineryId, buildPayload());
      history.push(machineriesRoute);
      return;
    }

    await createMachinery(buildPayload());
    history.push(machineriesRoute);
  };
  ...
  return (
    <>
      <MachineryForm
        ...
        submitLabel={editMode ? 'Edit Construction Machinery Listing' : 'Create Construction Machinery Listing'}
      />
...
export default MachineryCreatePage;
