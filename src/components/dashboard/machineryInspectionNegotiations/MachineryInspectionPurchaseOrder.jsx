...
import { getAcceptedMachineryInspectionOffer, getMachineryInspectionNegotiationTotals } from './machineryInspectionNegotiationUtils';

function formatCurrency(value) {
  return `Rs. ${Number(value || 0).toLocaleString()}`;
}

function MachineryInspectionPurchaseOrder({ basicInfo, reportBasePath, row }) {
  ...
  return (
    <>
      {row.machineryInspectionReport?._id ? (
        <div className="dashboard-form-actions mb-3">
          <Link className="dashboard-action-btn" to={`${reportBasePath}/${row.machineryInspectionReport._id}`}>View Inspection Report</Link>
        </div>
      ) : null}
          ...
          <table className="table truck-purchase-order__table">
            <thead>
              <tr>
                <th>Title</th>
                <th>Category</th>
                <th>Machinery Inspection Service Cost</th>
                <th>Total</th>
              </tr>
            </thead>
            <tbody>
...
export default MachineryInspectionPurchaseOrder;
