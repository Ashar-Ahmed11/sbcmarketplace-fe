...
import { getMachineryInspectionReportStatus } from './machineryInspectionNegotiations/machineryInspectionNegotiationUtils';
...
function MachineryInspectionNegotiationTable({ rows, title, subtitle, viewBasePath }) {
  return (
            ...
            {rows.length ? rows.map((item) => {
              ...
              return (
                <tr key={item._id}>
                  ...
                  <td className="py-3">{item.constructionMachinery?.title || '—'}</td>
                  ...
                  <td className="py-3"><span className={`status-badge ${getMachineryInspectionReportStatus(item).replace(/\s+/g, '-')}`}>{getMachineryInspectionReportStatus(item)}</span></td>
              ...
              <tr><td className="text-center py-4" colSpan="8">No machinery inspection negotiations found.</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
export default MachineryInspectionNegotiationTable;
