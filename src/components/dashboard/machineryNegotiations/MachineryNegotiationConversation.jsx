import { getAcceptedMachineryOffer } from './machineryNegotiationUtils';

function formatCurrency(value) {
  return `Rs. ${Number(value || 0).toLocaleString()}`;
}

function MachineryNegotiationConversation({
  currentUserId,
  onAccept,
  onOpenCounterOffer,
  row,
          ...
          return (
            <div className={`truck-negotiation-bubble ${isViewer ? 'buyer' : 'seller'}`} key={item._id}>
              <div className="truck-negotiation-bubble__inner">
                ...
                <strong>
                  {formatCurrency(item.machineryCost)}
...
export default MachineryNegotiationConversation;
