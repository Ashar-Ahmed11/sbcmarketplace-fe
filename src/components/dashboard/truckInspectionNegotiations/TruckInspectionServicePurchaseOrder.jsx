...
import { getAcceptedTruckInspectionServiceOffer, getTruckInspectionServiceNegotiationTotals } from './truckInspectionNegotiationUtils';

function formatCurrency(value) {
  return `Rs. ${Number(value || 0).toLocaleString()}`;
}

function TruckInspectionServicePurchaseOrder({ basicInfo, reportBasePath, row }) {
  ...
  return (
    <>
      {row.truckInspectionReport?._id ? (
        <div className="dashboard-form-actions mb-3">
          <Link className="dashboard-action-btn" to={`${reportBasePath}/${row.truckInspectionReport._id}`}>View Inspection Report</Link>
        </div>
      ) : null}
          ...
          <table className="table truck-purchase-order__table">
            <thead>
              <tr>
                <th>Title</th>
                <th>Category</th>
                <th>Truck Inspection Service Cost</th>
                <th>Total</th>
              </tr>
            </thead>
            <tbody>
...
export default TruckInspectionServicePurchaseOrder;
