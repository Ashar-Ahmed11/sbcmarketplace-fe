...
function RentalTruckNegotiationTable({ rows, title, subtitle, viewBasePath }) {
  return (
    <section className="dashboard-section-card">
      ...
      <div className="dashboard-table-wrap" style={{ border: '1px solid #E4E7EC' }}>
        <table className="table dashboard-table">
          <thead>
            <tr>
              <th className="py-3">Rental Truck</th>
          ...
          <tbody>
            {rows.length ? rows.map((item) => {
              ...
              return (
                <tr key={item._id}>
                  <td className="py-3">{item.rentalTruck?.title || 'Rental Truck Negotiation'}</td>
              ...
              <tr><td className="text-center py-4" colSpan="7">No rental truck negotiations found.</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
export default RentalTruckNegotiationTable;
