...
function SparePartNegotiationTable({ rows, title, subtitle, viewBasePath }) {
  return (
        ...
        <table className="table dashboard-table">
          ...
          <tbody>
            {rows.length ? rows.map((item) => {
              ...
              return (
                <tr key={item._id}>
                  <td className="py-3">{item.sparePart?.title || 'Spare Part Negotiation'}</td>
                  ...
                  <td className="py-3">{latest?.sparePartCost ? `Rs. ${Number(latest.sparePartCost).toLocaleString()}` : '—'}</td>
              ...
              <tr><td className="text-center py-4" colSpan="7">No spare part negotiations found.</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
export default SparePartNegotiationTable;
