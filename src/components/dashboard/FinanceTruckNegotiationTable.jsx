...
function FinanceTruckNegotiationTable({ rows, title, subtitle, viewBasePath }) {
  return (
        ...
        <table className="table dashboard-table">
          <thead>
            <tr>
              ...
              <th className="py-3">Latest Down Payment</th>
          ...
          <tbody>
            {rows.length ? rows.map((item) => {
              ...
              return (
                <tr key={item._id}>
                  <td className="py-3">{item.truck?.title || 'Truck Finance Negotiation'}</td>
                  ...
                  <td className="py-3">{latest?.downPayment ? `Rs. ${Number(latest.downPayment).toLocaleString()}` : '—'}</td>
            ...
            }) : <tr><td className="text-center py-4" colSpan="7">No finance truck negotiations found.</td></tr>}
          </tbody>
        </table>
      </div>
    </section>
...
export default FinanceTruckNegotiationTable;
