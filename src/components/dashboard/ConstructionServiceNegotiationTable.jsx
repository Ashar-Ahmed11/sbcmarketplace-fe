...
function ConstructionServiceNegotiationTable({ rows, title, subtitle, viewBasePath }) {
  return (
    <section className="dashboard-section-card">
      ...
      <div className="dashboard-table-wrap" style={{ border: '1px solid #E4E7EC' }}>
        <table className="table dashboard-table">
          <thead>
            <tr>
              <th className="py-3">Construction Service</th>
          ...
          <tbody>
            {rows.length ? rows.map((item) => {
              ...
              return (
                <tr key={item._id}>
                  <td className="py-3">{item.constructionService?.title || 'Construction Service Negotiation'}</td>
                  ...
                  <td className="py-3">{latest ? `Rs. ${Number(latest.labourCharges || 0).toLocaleString()}` : '—'}</td>
              ...
              <tr><td className="text-center py-4" colSpan="7">No construction service negotiations found.</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
export default ConstructionServiceNegotiationTable;
