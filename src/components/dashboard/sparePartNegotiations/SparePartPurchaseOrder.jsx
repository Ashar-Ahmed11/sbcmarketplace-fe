...
import { getAcceptedSparePartOffer, getSparePartNegotiationTotals } from './sparePartNegotiationUtils';

function formatCurrency(value) {
  return `Rs. ${Number(value || 0).toLocaleString()}`;
}

function SparePartPurchaseOrder({ basicInfo, row }) {
        ...
        <table className="table truck-purchase-order__table">
          <thead>
            <tr>
              <th>Title</th>
              <th>Category</th>
              <th>Spare Part Brand</th>
              <th>Spare Part Cost</th>
              <th>Total</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td>{row.sparePart?.title || '—'}</td>
              <td>{row.sparePart?.category?.name || '—'}</td>
              <td>{row.sparePart?.brand || '—'}</td>
              <td>{formatCurrency(totals.sparePartCost)}</td>
              <td>{formatCurrency(totals.sparePartCost)}</td>
            </tr>
            <tr>
              <td colSpan="3" />
...
export default SparePartPurchaseOrder;
