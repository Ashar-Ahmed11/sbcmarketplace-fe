...
import { getTruckInspectionReportStatus } from './truckInspectionNegotiations/truckInspectionNegotiationUtils';
...
function TruckInspectionNegotiationTable({ rows, title, subtitle, viewBasePath }) {
  return (
            ...
            {rows.length ? rows.map((item) => {
              ...
              return (
                <tr key={item._id}>
                  ...
                  <td className="py-3">{item.truck?.title || '—'}</td>
                  ...
                  <td className="py-3"><span className={`status-badge ${getTruckInspectionReportStatus(item).replace(/\s+/g, '-')}`}>{getTruckInspectionReportStatus(item)}</span></td>
              ...
              <tr><td className="text-center py-4" colSpan="8">No truck inspection negotiations found.</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
export default TruckInspectionNegotiationTable;
