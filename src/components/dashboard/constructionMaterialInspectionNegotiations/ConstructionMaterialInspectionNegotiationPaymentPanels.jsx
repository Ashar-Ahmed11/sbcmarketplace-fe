import {
  getConstructionMaterialInspectionNegotiationTotals,
  getConstructionMaterialInspectionReportStatus,
} from './constructionMaterialInspectionNegotiationUtils';
...
function ConstructionMaterialInspectionNegotiationPaymentPanels({
  acceptedOffer,
  basicInfo,
  isBuyer,
  onOpenAdvanceProof,
  ...
  const totals = getConstructionMaterialInspectionNegotiationTotals(acceptedOffer, basicInfo);
  const reportStatus = getConstructionMaterialInspectionReportStatus(row);
  const isReportApproved = row?.constructionMaterialInspectionReport?.status === 'approved';
      ...
      <section className="dashboard-section-card truck-negotiation-payment">
        <div className="truck-negotiation-payment__head">
          <div>
            ...
            <p>Pay advance fee to move this inspection request forward.</p>
          </div>
        ...
        <div className="truck-negotiation-payment__card">
          ...
          <PaymentRow label="Construction Material Inspection Service Cost" value={formatCurrency(totals.labourCharges)} />
  ...
  if (!isReportApproved) {
    return (
      <section className="dashboard-section-card truck-negotiation-payment">
        <div className="truck-negotiation-payment__head">
          <div>
            <h1>Inspection Report Status</h1>
            <p>The report must be created and approved before final payment becomes available.</p>
          </div>
          <span className={`status-badge ${reportStatus.replace(/\s+/g, '-')}`}>{reportStatus}</span>
        </div>
      </section>
    );
  }
    ...
    return (
      <section className="dashboard-section-card truck-negotiation-payment">
        <div className="truck-negotiation-payment__head">
          <div>
            <h1>Final Payment</h1>
            <p>Pay final payment to view the report and unlock the purchase order.</p>
          </div>
          <span className={`status-badge ${row.finalPaymentStatus}`}>{row.finalPaymentStatus}</span>
        </div>
        <div className="truck-negotiation-payment__card">
          <h2>Pay Final Amount</h2>
          <PaymentRow label="Agreed Construction Material Inspection Service Cost" value={formatCurrency(totals.labourCharges)} />
...
export default ConstructionMaterialInspectionNegotiationPaymentPanels;
