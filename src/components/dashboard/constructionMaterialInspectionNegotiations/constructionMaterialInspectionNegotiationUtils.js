...
export const getAcceptedConstructionMaterialInspectionOffer = (row) => (row?.negotiation || []).find((item) => item.accepted);

export const getConstructionMaterialInspectionNegotiationTotals = (offer, basicInfo) => {
...
export const getConstructionMaterialInspectionReportStatus = (row) => {
  if (!row?.constructionMaterialInspectionReport?._id) return 'pending';
  return row?.constructionMaterialInspectionReport?.status || 'pending approval';
};
